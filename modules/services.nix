  ...
  rosServicesToRestart, password, metadata, ... }:
{
  ...
  systemd.services.polyflow-webrtc = {
    ...
    after    = [ "network-online.target" "polyflow-wifi-mode.service" "sops-nix.service" ];
    wants    = [ "network-online.target" "polyflow-wifi-mode.service" "sops-nix.service" ];
    wantedBy = [ "multi-user.target" ];

    # Create a wrapper script that sources secrets from SOPS
    script = ''
      # Load secrets from SOPS if available, fallback to build-time defaults
      export ROBOT_ID=$(cat /run/secrets/ROBOT_ID 2>/dev/null || echo "${metadata.robotId}")
      export SIGNALING_URL=$(cat /run/secrets/SIGNALING_URL 2>/dev/null || echo "${metadata.signalingUrl}")
      export TURN_SERVER_URL=$(cat /run/secrets/TURN_SERVER_URL 2>/dev/null || echo "${metadata.turnServerUrl}")
      export TURN_SERVER_USERNAME=$(cat /run/secrets/TURN_SERVER_USERNAME 2>/dev/null || echo "${metadata.turnServerUsername}")
      export TURN_SERVER_PASSWORD=$(cat /run/secrets/TURN_SERVER_PASSWORD 2>/dev/null || echo "${metadata.turnServerPassword}")

      # Launch the webrtc node
